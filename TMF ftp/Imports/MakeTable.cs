            ...
            try
            {
                ...
                {   //RDS
                    ...
                    foreach (var q in query.ToList().Skip(1))
                    {
                        ...
                        row["LOW_BATTERY_ALR"] = Convert.ToInt32(q.LOW_BATTERY_ALR);
                        row["LEAK_ALR"] = Convert.ToInt32(q.LEAK_ALR);
                        row["MAGNETIC_TAMPER_ALR"] = Convert.ToInt32(q.MAGNETIC_TAMPER_ALR);
                        row["METER_ERROR_ALR"] = Convert.ToInt32(q.METER_ERROR_ALR);
                        row["BACK_FLOW_ALR"] = Convert.ToInt32(q.BACK_FLOW_ALR);
                        row["BROKEN_PIPE_ALR"] = Convert.ToInt32(q.BROKEN_PIPE_ALR);
                        row["EMPTY_PIPE_ALR"] = Convert.ToInt32(q.EMPTY_PIPE_ALR);
                        row["SPECIFIC_ERROR_ALR"] = Convert.ToInt32(q.SPECIFIC_ERROR_ALR);
                        row["MAC_ADDRESS"] = Regex.Replace(_gw, @"^(..)(..)(..)(..)(..)(..)$", "$1:$2:$3:$4:$5:$6");
